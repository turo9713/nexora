...
async function uploadProjectInput(file,workspaceId){const path=`/api/workbench/uploads?workspace_id=${encodeURIComponent(workspaceId)}&filename=${encodeURIComponent(file.name)}`;const response=await fetch(path,{method:"POST",credentials:"same-origin",headers:{"Accept":"application/json","Content-Type":file.type||"application/octet-stream","X-CSRF-Token":state.csrf},body:file});const data=await response.json().catch(()=>({error:"INVALID_RESPONSE"}));if(response.status===401){showLogin();throw new Error("UNAUTHORIZED")}if(!response.ok)throw new Error(data.message||data.error||"Файл не прошёл безопасную проверку");return data}
...
async function taskPage(id){title("Карточка задачи");activate("/tasks");const encoded=encodeURIComponent(id);const [item,timeline]=await Promise.all([api(`/api/tasks/${encoded}`),api(`/api/tasks/${encoded}/events`)]);clear();const c=card(`Задача ${item.task_id}`,true);c.classList.add("full");const badges=node("div",undefined,"actions");badges.append(badge(taskStatusLabel(item.status)),badge(`Агент: ${item.assigned_agent||"—"}`),badge(`Workflow: ${item.workflow||"—"}`));c.append(badges);const details=node("div",undefined,"detail-grid");[["Task ID",item.task_id],["Название",item.title],["Статус",item.status],["Этап",item.stage||"—"],["Прогресс",`${item.progress}%`],["Агент",item.assigned_agent||"—"],["Workflow",item.workflow||"—"],["Provider",item.provider_mode||"—"],["Создана",fmt(item.created_at)],["Обновлена",fmt(item.updated_at)],["Завершена",fmt(item.completed_at)],["Ошибка",item.error_code||"—"]].forEach(([k,v])=>{const d=node("div",undefined,"detail");d.append(node("small",k),node("strong",v));details.append(d)});c.append(details,node("h3","Описание"),node("p",item.description||"Описание не записано для этой задачи.","muted"),node("h3","Результат"),node("pre",item.result_summary||"Результат пока отсутствует"));const actions=node("div",undefined,"actions");if((item.downloads||[]).length){const download=node("a","Скачать безопасный результат .txt","button-link");download.href=`/api/tasks/${encoded}/downloads/result`;download.download=item.downloads[0].name;actions.append(download)}(item.artifacts||[]).forEach(artifact=>actions.append(artifactDownload(artifact)));if(item.status==="WAITING_APPROVAL"){const approvals=node("a","Открыть подтверждения","button-link");approvals.href="/approvals";approvals.dataset.route="/approvals";actions.append(approvals)}if(actions.childNodes.length)c.append(actions);c.append(node("h3","Timeline"));const events=timeline.items||[];if(!events.length)c.append(node("div","Lifecycle events пока отсутствуют.","empty"));events.forEach(event=>{const row=node("div",undefined,"approval-row");const info=node("div");info.append(node("strong",event.type),node("p",[event.status,event.stage,event.agent,event.workflow].filter(Boolean).join(" · ")||"Без дополнительных данных","muted"));row.append(info,node("small",fmt(event.timestamp),"muted"));c.append(row)});el("content").append(c)}

async function queuePage(){title("Queue & Workers");activate("/queue");clear();const summaryWrap=node("div",undefined,"grid");const tableWrap=node("div",undefined,"table-wrap");const notice=card("Постоянная очередь выполнения",true);notice.classList.add("full");notice.append(node("p","Read-only мониторинг: приоритет, попытки и состояние workers. Управление заданиями через эту страницу запрещено.","muted"));el("content").append(notice,summaryWrap,tableWrap);async function load(){const data=await api("/api/queue");summaryWrap.replaceChildren();[["В очереди",data.summary.queued],["Выполняются",data.summary.running],["Успешно",data.summary.succeeded],["Ошибки",data.summary.failed]].forEach(([label,value])=>summaryWrap.append(metric(label,value)));const table=node("table");const head=node("tr");["Job","Task","Название","Группа","Приоритет","Статус","Позиция","Попытка","Прогресс","Обновлено"].forEach(value=>head.append(node("th",value)));const thead=node("thead");thead.append(head);const body=node("tbody");data.items.forEach(item=>{const row=node("tr");row.append(node("td",item.job_id),node("td",item.task_id),node("td",item.title||"—"),node("td",item.worker_group),node("td",String(item.priority)));const status=node("td");status.append(badge(item.status));row.append(status,node("td",item.queue_position==null?"—":String(item.queue_position)),node("td",`${item.attempt}/${item.max_attempts}`),node("td",`${item.progress}%`),node("td",fmt(item.updated_at)));row.onclick=()=>navigate(`/tasks/${encodeURIComponent(item.task_id)}`);body.append(row)});table.append(thead,body);tableWrap.replaceChildren(data.items.length?table:node("div","Очередь пока пуста","empty"))}await load()}

function artifactSize(value){const bytes=Number(value)||0;if(bytes<1024)return `${bytes} Б`;if(bytes<1024*1024)return `${(bytes/1024).toFixed(1)} КБ`;return `${(bytes/1024/1024).toFixed(1)} МБ`}
function artifactDownload(item){const link=node("a",`Скачать ${item.kind.toUpperCase()}`,"button-link");link.href=`/api/artifacts/${encodeURIComponent(item.id)}/download?workspace_id=${encodeURIComponent(item.workspace_id)}`;link.download=item.name;return link}
async function artifactsPage(){
  title("Files & Artifacts Center");activate("/artifacts");
  const summary=await api("/api/dashboard");const workspaceId=summary.workspace.id;
  clear();const intro=card("Файлы и результаты",true);intro.classList.add("full");
  intro.append(node("p","Приватные результаты задач. Файлы изолированы текущим workspace, проверяются по checksum и выдаются только после авторизации.","muted"));
  const toolbar=node("form",undefined,"toolbar");const task=node("input");task.placeholder="Фильтр по Task ID";const submit=node("button","Применить","secondary");toolbar.append(task,submit);
  const list=node("div",undefined,"artifact-list");el("content").append(intro,toolbar,list);
  async function load(event){if(event)event.preventDefault();const query=new URLSearchParams({workspace_id:workspaceId});if(task.value.trim())query.set("task_id",task.value.trim());const data=await api(`/api/artifacts?${query}`);list.replaceChildren();data.items.forEach(item=>{const row=node("article",undefined,"artifact-row");const info=node("div");info.append(node("strong",item.name),node("p",`${item.task_id} · ${item.kind.toUpperCase()} · ${artifactSize(item.size_bytes)} · ${fmt(item.created_at)}`,"muted"));const actions=node("div",undefined,"actions");const preview=node("button","Предпросмотр","secondary");preview.type="button";preview.onclick=async()=>{preview.disabled=true;try{const details=await api(`/api/artifacts/${encodeURIComponent(item.id)}?workspace_id=${encodeURIComponent(workspaceId)}`);const content=node("pre",details.preview||"Предпросмотр недоступен","artifact-preview");row.querySelector(".artifact-preview")?.remove();row.append(content)}finally{preview.disabled=false}};actions.append(preview,artifactDownload(item));row.append(info,actions);list.append(row)});if(!data.items.length)list.append(node("div","Файлы пока не созданы. Заверши задачу в Workbench.","empty"))}
  toolbar.onsubmit=load;await load();
}
...
async function workbenchPage(){
  ...
  const sourceLabel=node("label",undefined,"project-upload");sourceLabel.append(node("strong","Исходные файлы"),node("small","До 5 файлов, каждый не более 8 МБ: TXT, MD, CSV, JSON, PDF, DOCX, XLSX, PNG, JPG","muted"));
  const sourceInput=node("input");sourceInput.type="file";sourceInput.multiple=true;sourceInput.accept=".txt,.md,.csv,.json,.pdf,.docx,.xlsx,.png,.jpg,.jpeg";
  const sourceList=node("div",undefined,"project-input-list");sourceLabel.append(sourceInput,sourceList);
  sourceInput.onchange=()=>{sourceList.replaceChildren();[...sourceInput.files].slice(0,5).forEach(file=>sourceList.append(badge(`${file.name} · ${artifactSize(file.size)}`)))};
  ...
  form.append(input,sourceLabel,submit,hint);
  form.onsubmit=async event=>{event.preventDefault();const message=input.value.trim();if(!message)return;const files=[...sourceInput.files];if(files.length>5){alert("Можно приложить не более пяти файлов");return}submit.disabled=true;submit.textContent="Загружаем исходные файлы…";try{const uploaded=[];for(const file of files){uploaded.push(await uploadProjectInput(file,summary.workspace.id))}submit.textContent="Создаём задачу…";const result=await api("/api/workbench/tasks",{method:"POST",body:JSON.stringify({message,workspace_id:summary.workspace.id,input_ids:uploaded.map(item=>item.id),idempotency_key:requestKey("task")})});navigate(`/workbench/tasks/${encodeURIComponent(result.task_id||result.id)}`)}catch(error){submit.disabled=false;submit.textContent="Запустить агентов";alert(error.message)}};
...
async function workbenchTaskPage(id){
  ...
  if((item.inputs||[]).length){const inputs=node("section",undefined,"project-inputs");inputs.append(node("h3","Исходные файлы"));item.inputs.forEach(source=>inputs.append(node("div",`${source.name} · ${artifactSize(source.size_bytes)}`,"project-input-row")));result.append(inputs)}
  ...
  (item.artifacts||[]).forEach(artifact=>{const link=artifactDownload(artifact);link.textContent=`Скачать ${artifact.kind.toUpperCase()} · v${artifact.version||1}`;actionBar.append(link)});
