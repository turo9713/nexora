...
from .artifact_content import ContentBlock, parse_artifact_content, structured_table_blocks
...
def render_docx(task: dict[str, Any]) -> bytes:
    ...
    if not deferred_tables and "xlsx" in selected_formats(title, result):
        deferred_tables.append(structured_table_blocks(content))
    if deferred_tables:
...
def render_pdf(task: dict[str, Any]) -> bytes:
    ...
    while index < len(content.blocks):
        ...
        index += 1
    if (
        not rendered_content_table
        and "xlsx" in selected_formats(str(task["title"]), str(task["result"]))
    ):
        rows = structured_table_blocks(content, limit=16)
        story.append(Paragraph("Структурированные данные", heading))
        table_data = [
            [
                Paragraph(
                    escape(value),
                    table_header if row.kind == "table_header" else table_body,
                )
        ...
        story.append(
            Table(
                table_data,
                colWidths=[1.25 * inch, 0.8 * inch, 2.35 * inch, 2.66 * inch],
                repeatRows=1,
                style=TableStyle(
                    [
                        ("FONTNAME", (0, 0), (-1, -1), font_name),
