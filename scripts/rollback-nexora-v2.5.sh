...
git -C "$root" rev-parse --verify v2.4.0^{commit} >/dev/null
stamp="$(date -u +%Y%m%dT%H%M%SZ)"
backup="${db}.pre-v2.5-rollback-${stamp}"
...
try:
    connection.executescript((root / "database/migrations/009_creator_economy.down.sql").read_text())
...
PY
chmod 600 "$backup" "$db"
echo "creator schema rollback complete; OpenClaw and VPS services were not changed"
echo "backup=$backup"
