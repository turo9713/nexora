...
git -C "$root" rev-parse --verify v2.3.0^{commit} >/dev/null

stamp="$(date -u +%Y%m%dT%H%M%SZ)"
backup="${db}.pre-v2.4-rollback-${stamp}"
...
down = (root / "database/migrations/008_marketplace.down.sql").read_text()
...
PY
chmod 600 "$backup" "$db"
echo "database rollback complete; application checkout remains administrator-controlled"
echo "backup=$backup"
